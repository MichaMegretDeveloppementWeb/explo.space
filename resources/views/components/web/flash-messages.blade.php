{{-- Toast notifications modernes (stack) --}}
...
@php
    ...
    if (session()->has('warning')) {
        ...
        foreach ($messages as $message) {
            $toasts[] = ['type' => 'warning', 'message' => $message];
        }
    }
    ...
    if (session()->has('info')) {
        ...
        foreach ($messages as $message) {
            $toasts[] = ['type' => 'info', 'message' => $message];
        }
    }
@endphp

@if(count($toasts) > 0)
    {{-- Container pour stack de toasts --}}
    <div class="fixed top-4 right-4 z-50 w-[90%] max-w-md space-y-3">
